...
import { Mail, RefreshCw, Search, Ticket, FileText, CheckCircle, Send, Phone, StickyNote } from 'lucide-react';
import { api } from '../../lib/api';
import { Lead, StaffMember, TicketSla, human, ago, fmtDate, staffName, Avatar, Drawer, SlaBadge } from './crmShared';
...
const KIND_ICON: Record<string, typeof Mail> = {
  enquiry: FileText, ticket: Ticket, ticket_update: Send, ticket_resolved: CheckCircle,
  lead_email: Mail, lead_call: Phone, lead_meeting: Phone, lead_whatsapp: Phone, lead_note: StickyNote,
};

export default function CustomersPanel({ staff, notify, focusEmail, onOpenLead, onOpenTicket }: {
  staff: StaffMember[];
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border border-border/80 bg-surface/70 overflow-x-auto">
        <table className="w-full text-left text-xs text-ink min-w-[720px]">
          ...
          <tbody className="divide-y divide-border/50">
            {shown.length === 0 && (
              <tr><td colSpan={6} className="p-8 text-center text-text-secondary">{loading ? 'Loading…' : 'No customers yet. They appear here as soon as someone sends an enquiry or raises a ticket.'}</td></tr>
            )}
            {shown.map((r) => (
