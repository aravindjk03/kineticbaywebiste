    ...
    <div className="fixed bottom-6 right-6 z-50 font-sans">
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="px-4 py-3 bg-surface/80 border-b border-border/80 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="relative w-12 h-12 -my-1 rounded-2xl bg-[radial-gradient(circle_at_50%_35%,#fffaf4,#f5c9a5)] overflow-hidden">
                  <Suspense fallback={<KaiAvatar className="w-10 h-10 m-1" />}>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3.5 scrollbar-thin scrollbar-thumb-border">
                  ...
                  >
                    <div
                      className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 text-xs font-semibold ${
                        isBot
                          ? 'bg-[#fbe3cc]/10 border border-primary/25'
                          : 'bg-surface-raised text-ink border border-border'
                      }`}
                    >
                      {isBot ? <KaiAvatar className="w-7 h-7" /> : <User className="w-4 h-4" />}
                    </div>
              ...
              {isTyping && (
                <div className="flex items-center gap-2.5">
                  <div className="w-8 h-8 rounded-xl bg-[#fbe3cc]/10 border border-primary/25 flex items-center justify-center shrink-0">
                    <KaiAvatar className="w-7 h-7" />
                  </div>
      ...
      <div className="relative flex justify-end">
        <AnimatePresence>
          {showHello && !isOpen && (
            <motion.button
              ...
              className="hidden sm:block absolute right-[72px] bottom-3 whitespace-nowrap px-4 py-2.5 bg-[#111214] text-[13px] text-ink shadow-[0_0_0_1px_rgba(240,138,75,0.45),0_14px_40px_-10px_rgba(0,0,0,0.9)] [clip-path:polygon(0_0,100%_0,100%_calc(100%_-_10px),calc(100%_-_10px)_100%,0_100%)]"
            >
        ...
        <motion.button
          ...
          className="kai-launcher relative w-[60px] h-[60px] rounded-[18px] bg-[radial-gradient(circle_at_50%_35%,#fffaf4_0%,#fbe3cc_55%,#f5b98f_100%)] shadow-[0_0_0_3px_rgba(240,138,75,0.35),0_20px_50px_-10px_rgba(240,138,75,0.65)]"
          aria-label={isOpen ? 'Close KAI, the Kinetic Bay assistant' : 'Chat with KAI, the Kinetic Bay assistant'}
        >
          <Suspense fallback={<KaiAvatar className="w-10 h-10 m-auto" />}>
            <ChatMascot mood={mood} zoom={1.1} className="absolute inset-0" />
          </Suspense>
          {isOpen && (
            <span className="absolute -top-1.5 -right-1.5 w-5 h-5 grid place-items-center rounded-full bg-bg text-primary shadow-[0_0_0_2px_#F08A4B]">
