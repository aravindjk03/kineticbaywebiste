...
class EdgeRequestBucketManager {
  constructor(options = {}) {
...
const edgeTicketSubmissionBucket = new EdgeRequestBucketManager({
...
const edgeTicketTrackingBucket = new EdgeRequestBucketManager({
...
const edgeMessageSubmissionBucket = new EdgeRequestBucketManager({
    ...
    if (url.pathname.startsWith('/api')) {
      try {
        ...
        if (path === '/api/public/tickets' && method === 'POST') {
          ...
          const fp = email ? `ticket_email:${email}` : null;

          const limitCheck = edgeTicketSubmissionBucket.checkLimit(clientIp, fp);
          if (!limitCheck.allowed) {
            return json({
              error: limitCheck.error,
              retryAfter: limitCheck.retryAfter,
              limitType: limitCheck.limitType,
            }, 429, limitCheck.headers);
          }
        ...
        if (path === '/api/public/ticket-status' && method === 'POST') {
          ...
          const fp = targetId ? `ticket_track:${targetId}` : null;

          const limitCheck = edgeTicketTrackingBucket.checkLimit(clientIp, fp);
          if (!limitCheck.allowed) {
            return json({
              error: limitCheck.error,
              retryAfter: limitCheck.retryAfter,
          ...
          if (t && t.requester_email.toLowerCase() === targetEmail) {
            ...
            }, 200, limitCheck.headers);
          }
          return json({ error: 'No ticket found matching the provided reference ID and requester email address.' }, 404, limitCheck.headers);
        }
        ...
        if (path === '/api/public/enquiries' && method === 'POST') {
          ...
          const body = await request.json().catch(() => ({}));
          const email = (body.email || '').trim().toLowerCase();
          const fp = email ? `msg_email:${email}` : null;

          const limitCheck = edgeMessageSubmissionBucket.checkLimit(clientIp, fp);
          if (!limitCheck.allowed) {
            return json({
              error: limitCheck.error,
              retryAfter: limitCheck.retryAfter,
